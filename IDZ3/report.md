...
---
## на 9 баллов
К предыдущей программе добавлен дополнительный процесс–наблюдатель, который интегрирует и корректно отображает все информацию, ранее разработанных процессов независимых процессов: `observe.c`.
### 1. Завершение работы:
Для завершения приложения используются те же подходы, что и в программах на меньший балл: основной сценарий завершения - по Ctrl+C.
### 2. Запуск процессов:
Все процессы запускаются независимо друг от друга, каждый в своей консоли. Для интеграции информации от всех процессов используется именованный канал FIFO: каждый процесс-читатель и процесс-писатель отправляет свои сообщения не только в собственную консоль, но и через именованный канал записывает их для наблюдателя. Процесс-наблюдатель читает этот канал и отображает работы всех остальных процессов.
### 3. Инструкция по запуску и результаты работы:
1. Открываем четыре терминала  и во всех терминалах собираем файлы:
```
gcc init.c    -o init    -pthread
gcc reader.c  -o reader  -pthread
gcc writer.c  -o writer  -pthread
gcc observer.c -o observer -pthread
```
2. В одном из терминалов запускаем сначала инициализатор:
```
./init
```
3. Во втором запускаем процесс-наблюдатель:
```
./observer
```
5. В одном из оставшихся запускаем процесс-писатель, в другом - читатель:
```
./reader
```
```
./writer
```
5. Можно еще пооткрывать терминалов, собрать в них три файла и дальше запустить еще процессов читателей и писателей.

Результаты работы `init.c` и `observer.c`:
<img width="1142" height="373" alt="Снимок экрана 2025-11-30 в 16 57 49" src="https://github.com/user-attachments/assets/c76465a9-d82e-47a8-951d-39f9e11a6215" />

Результаты работы `writer.c` и `reader.c`:
<img width="1144" height="376" alt="Снимок экрана 2025-11-30 в 16 58 09" src="https://github.com/user-attachments/assets/c4200e1b-5486-45d3-b3a0-77716d61beca" />

---
## на 10 баллов
Программа, допускающая подключение не одного, а несколько наблюдателей, каждый из которых выводит информацию в свою консоль.
### 1. Завершение работы:
Для завершения приложения используются те же подходы, что и в программах на меньший балл: основной сценарий завершения - по Ctrl+C.
### 2. Наблюдатели:
Программа допускает создание до 4-х наблюдателей, все наблюдатели работают одинаково и согласовнно. Каждый из наблюдателей при запуске получает уникальный идентификатор через аргумент командной строки и открывает для себя отдельный именованный канал. Это позволяет любому количеству наблюдателей независимо подключаться к процессам‑читателям и процессам-писателям, не мешая друг другу.
### 3. Инструкция по запуску и результаты работы нескольких наблюдателей:
1. Открываем терминалы для инициализатора, писателей, читателей и наблюдателей и во всех терминалах собираем файлы:
```
gcc init.c    -o init    -pthread
gcc reader.c  -o reader  -pthread
gcc writer.c  -o writer  -pthread
...
./init
```
3. Далее запускаем наблюдатели сколько нужно в разных терминалах(от 1 до 4), цифра в конце это номер канала:
```
./observer 1
```
5. Далее запускаем читателей и писателей:
```
./reader
```
```
./writer
```
Результаты работы двух наблюдателей:
<img width="1141" height="370" alt="Снимок экрана 2025-11-30 в 21 17 07" src="https://github.com/user-attachments/assets/3a355d15-7461-47d4-9515-0f93a3874de8" />
