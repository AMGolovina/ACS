...
P.S. ожидаемая оценка ... баллов, так что при описании на баллы ниже может отличатся немного структура и реализация)))
...
- в `reader_process(int id)` читатель читает `shared->db[idx]` и использует общие поля `read_count`, `terminate`.
```
...
shared->read_count--;
```
- в `writer_process(int id)` писатель читает и меняет элементы массива `shared->db` в той же общей памяти.
```
int idx = rand() % DB_SIZE;
