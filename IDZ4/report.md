...
Условные переменные применяются для организации ожидания: читатели ожидают возможность чтения, если в данный момент активен писатель или есть ожидающие писатели; писатели ожидают возможность записи, пока есть активные читатели или другой писатель. Мьютекс используется совместно с условными переменными для защиты числа активных читателей, наличия активного писателя, числа ожидающих писателей и для корректной работы pthread_cond_wait. Атомарная переменная (atomic_int stop_flag) используется как общий потокобезопасный признак завершения работы.

### 2. Сравнительный анализ:

outMain.log - вывод для задачи на 8 баллов

outAnother.log - вывод для задачи альтернативным решением

<img width="1142" height="372" alt="Снимок экрана 2025-12-15 в 02 40 13" src="https://github.com/user-attachments/assets/bb32944b-d691-490a-a888-8bb4487f337a" />

При одинаковых исходных данных (одинаковые N, K и одинаковая начальная БД/seed) обе версии демонстрируют одинаковую семантику задачи «читатели–писатели»: читатели выполняют только чтение, писатели выполняют транзакцию «изменение + сортировка», и чтение не пересекается с записью. Полного совпадения порядка строк нет, поэтому идентичность подтверждается соблюдением диапазонов (idx 0..19, new 1..1000) и сохранение непротиворечивости БД после каждой записи.
